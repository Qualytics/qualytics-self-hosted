...
- **Extra Packages**: Teradata + IBM DB2 JDBC drivers, passed to spark-submit via `--packages` and resolved by Ivy (`spark.jars.ivy=/tmp`) at driver startup — i.e. from Maven Central on every driver start, not from the image (vendor licensing forbids bundling them in the shadowJar/image). Mirroring container images does not cover these. Air-gapped/egress-restricted installs set `dataplane.ivySettingsSecret` (pre-existing Secret with an `ivysettings.xml` key) — mounted into the driver at `/opt/spark/conf/ivysettings.xml` and passed via `spark.jars.ivySettings`, it replaces the default resolvers entirely (Maven Central never contacted) and redirects resolution to an internal Maven repository (Artifactory/Nexus). Driver-only egress; executors get resolved jars from the driver. See [docs/custom-maven-repository.md](docs/custom-maven-repository.md).
...
- **Sections**:
  ...
  8. Dataplane configuration (Spark settings, including `dataplane.rbac.{create,serviceAccountName,serviceAccountAnnotations}`, `ivySettingsSecret`, `maxParallelSyncRequests`, and the `syncAdmission` CPU governor)
...
- [docs/custom-maven-repository.md](docs/custom-maven-repository.md) — BYO `ivysettings.xml` Secret (`dataplane.ivySettingsSecret`) to resolve `extraPackages` from an internal Artifactory/Nexus instead of Maven Central
