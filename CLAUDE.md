...
- **Hive support**: `MOTHERSHIP_SPARK_ENABLE_HIVE=true` is hardcoded on the driver, because the dataplane image leaves Spark Hive support off unless it is set, and every Hive Native datastore fails its connection test without it. Deliberately not a value: the dataplane is dropping the flag and will always enable Hive support. Metastore addresses and keytabs stay on each connection.
...
- **Sections**:
  ...
  8. Dataplane configuration (Spark settings, including `dataplane.rbac.{create,serviceAccountName,serviceAccountAnnotations}`, `dataplane.ivy.*`, `maxParallelSyncRequests`, and the `syncAdmission` CPU governor)
