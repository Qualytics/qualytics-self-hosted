...
- **Extra Spark conf**: `dataplane.extraSparkConf` (map, default `{}`) renders one single-quoted `--conf 'key=value'` per entry, sorted by key, after the chart's own confs and immediately before the application jar, so values reach spark-submit verbatim and a key the chart already sets is replaced. It covers settings no dedicated value exposes, typically Hadoop client settings (`spark.hadoop.*`) such as an HA HDFS nameservice or RPC protection for a Kerberized cluster. The `qualytics.spark.extraConfArg` helper fails the render for keys outside `spark.*` (spark-submit silently ignores them) and for `spark.driver.bindAddress` / `spark.submit.deployMode` (entrypoint-owned, invariant 2 below).
